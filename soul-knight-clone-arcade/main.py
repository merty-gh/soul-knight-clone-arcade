...
from entities.items import CrystalDrop, Chest, ShieldItem, WeaponDrop
...
class GameWindow(arcade.Window):
    ...
    def draw_game(self):
        ...
        for item in self.level_generator.item_list:
            if isinstance(item, Chest):
                dist = arcade.get_distance_between_sprites(self.player, item)
                if dist < 100:
            ...
            if key == arcade.key.E:
                if self.level_generator is not None:
                    ...
                    if closest_chest:
                        ...
                        self.physics_engine = arcade.PhysicsEngineSimple(self.player, all_obstacles)
    ...
    def update_room_logic(self):
        ...
        if (buffer < local_x < config.ROOM_WIDTH_PX - buffer) and (buffer < local_y < config.ROOM_HEIGHT_PX - buffer):
    ...
    def on_enter_room(self, room_coords):
        ...
        if not room_data or room_data['cleared']:
            if room_data:
                for door in room_data['doors']: door.center_x = -10000
            return
    ...
    def start_next_wave(self, room_coords):
        ...
        for _ in range(count):
            ...
            if wave_num == 1:
                enemy = Zombie(ex, ey, self.player) if random.random() < 0.7 else Slime(ex, ey, self.player)
            elif wave_num == 2:
                r = random.random()
                enemy = Zombie(ex, ey, self.player) if r < 0.5 else (
                    Bat(ex, ey, self.player) if r < 0.8 else Slime(ex, ey, self.player))
            else:
                r = random.random()
                if r < 0.4:
                    ...
                    enemy = Bat(ex, ey, self.player)
                else:
                    enemy = Slime(ex, ey, self.player)
