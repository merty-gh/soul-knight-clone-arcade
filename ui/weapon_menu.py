...
class WeaponMenu:
    ...
    def setup_buttons(self):
        ...
        for i, weapon_data in enumerate(config.WEAPONS_CONFIG):
            ...
            # Проверяем разблокировку по class_name, так как именно это сохраняется в БД
            is_unlocked = weapon_data["class_name"] in self.unlocked_weapons or price == 0
    ...
    def draw(self):
        ...
        for idx, btn in enumerate(self.buttons):
            ...
            text_str = f"{data['price']} Crystals"
    ...
    def handle_click(self, btn):
        ...
        if btn["unlocked"]:
            # Оружие уже разблокировано, просто экипируем
            self.player.equip_weapon_by_name(data["class_name"])
            self.refresh_state()
        else:
            # Пытаемся купить оружие
            # Сначала обновляем кристаллы из БД на случай изменений
            self.player.update_crystals_from_db()
            
            if self.player.crystals >= data["price"]:
                # Сначала тратим кристаллы, и только если успешно — разблокируем
                if database.spend_crystals(data["price"]):
                    database.unlock_weapon(data["class_name"])
                    self.player.update_crystals_from_db()
                    self.player.equip_weapon_by_name(data["class_name"])
                    self.refresh_state()
    ...
    def refresh_state(self):
        ...
        for btn in self.buttons:
            ...
            # Проверяем разблокировку по class_name
            btn["unlocked"] = weapon_class in self.unlocked_weapons or btn["data"]["price"] == 0
